...
cropPub = rospy.Publisher("omnicam_cropped", CompressedImage, queue_size=1)
...
def specialCrop(feedtocrop, x1, y1, w1, h1):
    ...
    if x1 - w1 / 2 < 0:
        ...
        roi[0:roi.shape[0], w1 / 2 + sizein_x - x1:roi.shape[1]] = feedtocrop[y1 - h1 / 2:y1 + h1 / 2,
                                                                   0:w1 / 2 + x1 - sizein_x + 1]
...
def callback(data):
    ...
    msg.format = "jpeg"
    msg.data = np.array(cv2.imencode('.jpg', cropped)[1]).tostring()
