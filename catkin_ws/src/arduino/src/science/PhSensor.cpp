...
float PhSensor::get_pH(char command) {
    if(command=='c'||command=='r')time_=1800; //if a command has been sent to calibrate or take a reading we wait 1800ms so that the circuit has time to take the reading.
