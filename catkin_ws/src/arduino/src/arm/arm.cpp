...
//    sender.sendTransforms();
    ...
    if (pid) {
        nodeHandle.logerror("Position control not working, switching to speed control");
        pid = false;
...
void handle_arm_position(const arm_control::JointPosition & message) {
    ...
    *pitch1SetPoint = message.base_pitch;
    arm::PitchRollCompute::inversePosition(message.diff_1_pitch, message.diff_1_roll,
                                  diff1setPointLeft, diff1setPointRight);
    arm::PitchRollCompute::inversePosition(message.diff_2_pitch, message.diff_2_roll,
