...
class ArmTf(object):
    ...
    def callback(self, message):
        ...
        self.tf_broadcaster.sendTransform((self.armPosition, 0, 0), base_yaw_quat,
                                          rospy.Time.now(), self.baseYawFrame, self.baseFrame)
        self.tf_broadcaster.sendTransform((0, 0, self.pitch1offset), base_pitch_quat,
                                          rospy.Time.now(), self.pitch1Frame, self.baseYawFrame)
        self.tf_broadcaster.sendTransform((self.pitch2offset, 0, 0), diff_1_pitch_quat,
                                          rospy.Time.now(), self.pitch2Frame, self.pitch1Frame)
        self.tf_broadcaster.sendTransform((self.roll1offset, 0, 0), diff_2_pitch_quat,
                                          rospy.Time.now(), self.roll1Frame, self.pitch2Frame)
        self.tf_broadcaster.sendTransform((self.pitch3offset, 0, 0), diff_1_roll_quat,
                                          rospy.Time.now(), self.pitch3Frame, self.roll1Frame)
        self.tf_broadcaster.sendTransform((self.roll2offset, 0, 0), diff_2_roll_quat,
                                          rospy.Time.now(), self.roll2Frame, self.pitch3Frame)
